import type { Service } from '$lib/data/service.model'

export function template(serviceId: string, operationName: string) {
  return {
    serviceId,
    operationName,
    array: [1, 2, 3],
    boolean: true,
    color: '#82b92c',
    null: null,
    number: 123,
    object: { a: 'b', c: 'd' },
    string: 'Hello World',
  }
}

export function response(serviceId: string, operationName: string) {
  return {
    serviceId,
    operationName,
    result: 'some result',
  }
}

export const services: Service[] = [
  {
    id: '5eb4a8c808dfe857ed9e388b',
    name: 'NikudaApi',
    operations: [
      {
        name: 'getInsurety',
      },
      {
        name: 'isTelpod',
      },
      {
        name: 'isIsotronic',
      },
      {
        name: 'isInsectus',
      },
      {
        name: 'setGeekola',
      },
      {
        name: 'getSchoolio',
      },
      {
        name: 'setCorecom',
      },
      {
        name: 'getBrainclip',
      },
      {
        name: 'getWrapture',
      },
      {
        name: 'isGeekosis',
      },
    ],
  },
  {
    id: '5eb4a8c87e7b429b81231baa',
    name: 'VelityApi',
    operations: [
      {
        name: 'getQuarx',
      },
      {
        name: 'isAssitia',
      },
      {
        name: 'setBiotica',
      },
      {
        name: 'isStralum',
      },
      {
        name: 'setEcolight',
      },
      {
        name: 'isKraggle',
      },
      {
        name: 'setSensate',
      },
      {
        name: 'isNamebox',
      },
      {
        name: 'isRoboid',
      },
      {
        name: 'getVetron',
      },
    ],
  },
  {
    id: '5eb4a8c85536b82aa5cad4cb',
    name: 'LyrichordApi',
    operations: [
      {
        name: 'setQuility',
      },
      {
        name: 'getZerology',
      },
      {
        name: 'setZanilla',
      },
      {
        name: 'setDuoflex',
      },
      {
        name: 'setUpdat',
      },
      {
        name: 'isBittor',
      },
      {
        name: 'setZytrek',
      },
      {
        name: 'getZolavo',
      },
      {
        name: 'setZyple',
      },
      {
        name: 'isAquazure',
      },
    ],
  },
  {
    id: '5eb4a8c8f2ea46cebe38899a',
    name: 'SynkgenApi',
    operations: [
      {
        name: 'getPolarax',
      },
      {
        name: 'getGeeknet',
      },
      {
        name: 'setZentry',
      },
      {
        name: 'getMoltonic',
      },
      {
        name: 'isZentury',
      },
      {
        name: 'setEdecine',
      },
      {
        name: 'setComvey',
      },
      {
        name: 'getZuvy',
      },
      {
        name: 'isRodeology',
      },
      {
        name: 'getObliq',
      },
    ],
  },
  {
    id: '5eb4a8c85abcb4839a5365cb',
    name: 'ShepardApi',
    operations: [
      {
        name: 'getMarqet',
      },
      {
        name: 'isTalae',
      },
      {
        name: 'setVortexaco',
      },
      {
        name: 'isTurnling',
      },
      {
        name: 'setWaretel',
      },
      {
        name: 'getEntropix',
      },
      {
        name: 'setChorizon',
      },
      {
        name: 'getComstar',
      },
      {
        name: 'isProgenex',
      },
      {
        name: 'getWaterbaby',
      },
    ],
  },
  {
    id: '5eb4a8c8b08100ebfccb8fc2',
    name: 'CombogenApi',
    operations: [
      {
        name: 'setAquoavo',
      },
      {
        name: 'getZounds',
      },
      {
        name: 'isSolaren',
      },
      {
        name: 'isBaluba',
      },
      {
        name: 'getBesto',
      },
      {
        name: 'setQuilm',
      },
      {
        name: 'setMinga',
      },
      {
        name: 'isTemorak',
      },
      {
        name: 'getAnivet',
      },
      {
        name: 'getExtragen',
      },
    ],
  },
  {
    id: '5eb4a8c80d631f0a1b5e21fd',
    name: 'IsologixApi',
    operations: [
      {
        name: 'isAnixang',
      },
      {
        name: 'isBuzzopia',
      },
      {
        name: 'setTerrago',
      },
      {
        name: 'isBoilicon',
      },
      {
        name: 'setPortaline',
      },
      {
        name: 'setPyramis',
      },
      {
        name: 'setGeekol',
      },
      {
        name: 'setXumonk',
      },
      {
        name: 'setCenturia',
      },
      {
        name: 'setBizmatic',
      },
    ],
  },
  {
    id: '5eb4a8c8fa093d446934716c',
    name: 'ExostreamApi',
    operations: [
      {
        name: 'isRocklogic',
      },
      {
        name: 'isApexia',
      },
      {
        name: 'setEnersol',
      },
      {
        name: 'isYogasm',
      },
      {
        name: 'getAeora',
      },
      {
        name: 'isCombogene',
      },
      {
        name: 'isLyria',
      },
      {
        name: 'isOverplex',
      },
      {
        name: 'getEnaut',
      },
      {
        name: 'setQaboos',
      },
    ],
  },
  {
    id: '5eb4a8c8a81ce48b14edd5b7',
    name: 'PlexiaApi',
    operations: [
      {
        name: 'getTurnabout',
      },
      {
        name: 'setOlympix',
      },
      {
        name: 'getMusix',
      },
      {
        name: 'setIsbol',
      },
      {
        name: 'getBoilcat',
      },
      {
        name: 'isAclima',
      },
      {
        name: 'getFiberox',
      },
      {
        name: 'isSentia',
      },
      {
        name: 'getKyagoro',
      },
      {
        name: 'isMaximind',
      },
    ],
  },
  {
    id: '5eb4a8c8853e753d34f9d8a8',
    name: 'VisualixApi',
    operations: [
      {
        name: 'isMagneato',
      },
      {
        name: 'getZepitope',
      },
      {
        name: 'isBullzone',
      },
      {
        name: 'getSupportal',
      },
      {
        name: 'isXymonk',
      },
      {
        name: 'getPyramia',
      },
      {
        name: 'isFleetmix',
      },
      {
        name: 'isTropoli',
      },
      {
        name: 'isNetur',
      },
      {
        name: 'setNimon',
      },
    ],
  },
]
